...
### 修复
- **文件浏览器 Gitignore 规则修复（Issue #147 / PR #156）**：项目根 `.gitignore` 改用标准 Gitignore 语义匹配，修复嵌套目录规则、根路径锚点、通配符和否定规则判断错误；创建、修改或删除 `.gitignore` 时会自动重载，文件缺失或不可读时继续使用内置默认规则，忽略目录仍归入“已折叠文件”，忽略文件保持隐藏。
